...
public class Lv_1_03 {
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(st.solution(park, routes)));

		String[] park2 = {"SOO","OXX","OOO"};
		String[] routes2= {"E 2","S 2","W 1"};
		System.out.println(Arrays.toString(st.solution(park2, routes2)));

		String[] park3 = {"OSO","OOO","OXO","OOO"};
		String[] routes3 = {"E 2","S 3","W 1"};
		System.out.println(Arrays.toString(st.solution(park3, routes3)));
	}
    	...
    	for(int i=0; i<routes.length; i++) {
	    	...
	    	for(int k=0; k<2; k++) {
				...
				if(k % 2 == 0) {
					...
					moving = Integer.parseInt(move[k]);
					try {
						for(int j=0; j<moving; j++) {
							if(direction.equals("E")) {
								if(arr[answer[0]][answer[1]+1] == 'X') {
								break;
							}
								answer[1] += moving;
	    	...
	    	if(answer[0] < 0 || answer[1] < 0 || answer[0] >= arr.length || answer[0] >= arr[0].length) {
	    		answer = temp.clone();
	    	}
		}
