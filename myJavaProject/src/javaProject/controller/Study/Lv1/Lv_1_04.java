...
public class Lv_1_04 {
	...
	public static void main(String[] args) {
		System.out.println("대충 만든 자판");
		Solution4 st = new Solution4();
		String[] keymap = {"ABACD", "BCEFD"};
		String[] targets = {"ABCD","AABB"};
		System.out.println(Arrays.toString(st.solution(keymap, targets)));

		String[] keymap2 = {"AA"};
		String[] targets2 = {"B"};
		System.out.println(Arrays.toString(st.solution(keymap2, targets2)));

		String[] keymap3 = {"AGZ", "BSSS"};
		String[] targets3 = {"ASA","BGZ"};
		System.out.println(Arrays.toString(st.solution(keymap3, targets3)));
...
class Solution4 {
    public int[] solution(String[] keymap, String[] targets) {
        ...
        // 키를 하나로 만든다. "ABACD", "BCEFD" 예로 A:1, B:1, C:2, 이런식으로 맵에 넣는다.
        for (String s: keymap) {
...
//        int[] answer = new int[targets.length];
//        int[] search = new int[keymap.length];
//        int idx = 0;
//        
//        for(int i=0; i<targets.length; i++) {
...
//                        continue;
//                    } else {
//                    	search[k] = keymap[k].indexOf(ch) +1;
//                    	idx = search[k];
//                    } 
//                    if(k != 0 && keymap.length > 1) {
//                    	idx = (int) (search[k] < search[k-1]? search[k]:search[k-1]);
//                    }
//                }
//                if(chk == keymap.length) {
//                	answer[i] = -1;
//                	break;
//                } else {
//                	answer[i] += idx;
